...
def index():
    ...
    if request.method == 'POST':
        ...
        device_info = {
            'ProductName': request.form.get('product_name', 'Unknown Product'),
            'ProductVersion': request.form.get('product_version', 'Unknown Version'),
            'ProductCategory': request.form.get('product_category', 'Unknown Category'),
            'CompanyName': request.form.get('company_name', 'Unknown Company'),
        ...
        xml_content = generate_xml(device_info, selected_test_cases, additional_config_test_cases, additional_settings)
...
def generate_xml(device_info, selected_cases, additional_config_test_cases, additional_settings):
    ...
    for key in ['ProductName', 'ProductVersion', 'ProductCategory', 'CompanyName']:
        SubElement(device_info_el, key).text = device_info.get(key, '') or 'NULL'

    # DeviceConfiguration 추가 (endpoint, username, password 포함)
    device_config = SubElement(root, 'DeviceConfiguration')
    SubElement(device_config, 'Endpoint').text = device_info.get('endpoint', '') or 'NULL'
    auth = SubElement(device_config, 'Authentication')
    SubElement(auth, 'Username').text = device_info.get('username', '') or 'NULL'
    SubElement(auth, 'Password').text = device_info.get('password', '') or 'NULL'
    ...
    for case, settings in additional_settings.items():
        if case in sum(additional_config_test_cases.values(), []) and settings:
